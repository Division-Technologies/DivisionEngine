...
| ゾーン | グループ | 由来 | 意味 |
|---|---|---|---|
| `EE suspend: <理由>` | `suspend` | `RuntimeSuspendStarted` → `RuntimeResumeFinished` | ストップ・ザ・ワールド全体。止める側のスレッドに出る。理由は GC / GC prep / rejit / shutdown 等に分かれる |
| `EE stopped` | `suspend` | `RuntimeSuspendFinished` → `RuntimeResumeStarted` | 全スレッドが実際に止まっていた区間。「止めるのに手間取った」と「止まっていた時間が長い」を分ける |
| `EE wait: <理由>` | `gc` | `RuntimeThreadSuspended` → `RuntimeThreadResumed` | そのスレッドがサスペンドのせいでマネージドコードを走らせられなかった区間。止められた側の各スレッドに出る |
| `GC gen0/1/2` | `gc` | `GarbageCollectionStarted` → `Finished` | スレッドではなく `GC` という名前の独立したレーンに出る。誘発された GC は `GC genN (induced)` |
| `JIT` | `jit` | `JITCompilationStarted` → `Finished` | FunctionID を値として持つ。定常状態に出るものは段階的 JIT の再コンパイル |
| `Module load` | `loader` | `ModuleLoadStarted` → `Finished` | モジュール名をテキストに付ける |
...
### GC で止まっていた時間をスレッドごとに出す

`EE suspend` と `EE stopped` は止める側のスレッド（ワークステーション GC なら GC を起こしたスレッド、
Server GC ならヒープ 0 の GC スレッド）に 1 本出るだけで、
ワーカーのタイムライン上には「なぜかここで何もしていない」隙間としてしか現れない。
知りたいのは、各スレッドが GC のせいで止まっていた区間を、そのスレッドの上にゾーンとして見ることである。

それが `EE wait: <理由>` で、`RuntimeThreadSuspended` / `RuntimeThreadResumed`（`COR_PRF_MONITOR_SUSPENDS`）から作る。
このコールバックの出どころは `vm/threadsuspend.cpp` の 3 か所で、呼ばれるスレッドがそれぞれ違う
（dotnet/runtime v10.0.0 のソースで確認）。

| 出どころ | コールバックが走るスレッド | 扱い |
|---|---|---|
| `Thread::RareDisablePreemptiveGC` | 止められるスレッド自身。マネージドに戻ろうとした、またはセーフポイントに連れてこられたスレッドが、`WaitUntilGCComplete` の前後で Suspended / Resumed を自分で送る | ゾーンにする |
| `ThreadSuspend::SuspendEE` / `RestartEE` | 止める側のスレッド自身（GC 専用スレッドはランタイムが除外する） | ゾーンにする。ワークステーション GC では GC を起こしたスレッドに GC 全体が出る |
| `Thread::SuspendThread` / `ResumeThread`（Windows のみ） | 止める側。他スレッドを一瞬 OS レベルで止めてリダイレクトするときに、相手の ThreadID で呼ばれる | 無視する |

ThreadID はランタイムの `Thread` オブジェクトそのものなので、`ICorProfilerInfo::GetCurrentThreadID` と比べて
「自分宛て」のときだけゾーンを開閉する。
他スレッド宛ての通知でそのスレッドのゾーンを開くことはできない（Tracy のゾーンは開いたスレッドでしか閉じられない）。
`GetCurrentThreadID` はロックも確保もしない呼び出しなので、他スレッドが OS レベルで止まっている最中でも安全に呼べる。
同じ理由で、この経路はゾーンスタックを伸ばさない（スレッドごとに 16 段分をインラインで持ち、足りなければゾーンを捨てる）。

自分宛ての Suspended / Resumed は同じスレッドの同じ呼び出しの中で対になって来るので、スレッドごとのスタックで扱える。
例外は 1 つで、`RareDisablePreemptiveGC` はデバッガもそのスレッドを止めようとしているときに Suspended だけを送らない。
対応するゾーンが無い Resumed は何もしない。

理由（GC / GC prep / rejit …）はこのコールバックには来ないので、`RuntimeSuspendStarted` で受けた理由を覚えておいて使う。
`SuspendEE` は理由を通知してからスレッドのトラップを立てるので、止まったスレッドは今回の理由を読める。

見えないものもある。
サスペンドの間ずっとプリエンプティブモードにいたスレッド（ネイティブコードでブロックしていた、など）には通知が無い。
止める必要が無かったのだから、ある意味で正しい。
ただし Server GC で GC を起こしたスレッド自身もこれに当たる。
`wait_for_gc_done` でプリエンプティブのまま GC の終わりを待つので、`EE wait` が出ない。

### GC はスレッドではなくレーンに出す

`GarbageCollectionStarted` と `GarbageCollectionFinished` は同じスレッドに来るとは限らない。
最初の実装はこれを見落として GC をスレッド上のゾーンにしており、`DOTNET_gcServer=1` で壊れた。
開いたゾーンが閉じないまま残り、その後のゾーンが全部その下に入れ子になって、スタックの上限まで積み上がる。

`gc/gc.cpp`（v10.0.0）で確かめると、ずれ方は 2 通りある。

- Server GC では、開始（`garbage_collect` の中の `DiagGCStart`）は「世代決定」の join に最後に着いた GC スレッドが、
  終了（`do_post_gc` の中の `DiagGCEnd`）はヒープ 0 の GC スレッドが送る
- バックグラウンド GC では、開始は GC を起こしたスレッドが、終了はバックグラウンド GC スレッドが送る。こちらはワークステーション GC でも起きる

Tracy の CPU ゾーンは開いたスレッドでしか閉じられないので、GC をスレッドに載せる形はどう直しても成り立たない。
そこで GC は Tracy の GPU コンテキストに出している。
スレッドに縛られない GPU コンテキストはゾーンスタックを 1 本だけ持ち、開始と終了をどのスレッドから送ってもよく、時刻も呼び出し側が渡す。
レンダラの GPU タイムラインのための仕組みだが、「スレッドに属さない区間」を描く道具としてそのまま使える。
タイムライン上では `GC` という名前のレーンになり、統計も GPU ゾーンとして出る。

- 時刻はこちらの `steady_clock` のナノ秒を渡し、GC のたびに `___tracy_emit_gpu_time_sync_serial` で Tracy の時計と合わせ直す
- イベントは Tracy の直列化キュー（ロックを取る）を通る。GC は 1 フレームに数回なので問題にならない
- コンテキスト ID は 255 を使う。C API では ID は呼び出し側が選び、C++ API は 0 から配るので、上端なら衝突しない
- `TRACY_ON_DEMAND` では、UI が繋がっていない間に始まった GC の終了は送らない。開始を見ていない終了を UI は受け付けないため

`TRACY_FIBERS` でもゾーンをスレッド間で持ち回せるが、採らなかった。
fiber を有効にするとプロセス中の全ゾーンの begin / end が同じ直列化キューを通るようになり（`TracyQueuePrepare` の定義が変わる）、
エンジン側のゾーンまでロックを取るようになる。
並列スケジューラの待ちを見るための計装に、それは入れられない。
なお `___tracy_c_zone_context` の形は fiber の有無で変わらないので、`TracyNative.cs` との ABI は理由ではない。

終了のコールバックはどの GC が終わったかを言わない。
GC は入れ子になりうる（バックグラウンド gen2 の最中にエフェメラル GC が走る）ので、終了は一番内側を閉じる。
また、ランタイムは開始より終了を多く送ることがある。
バックグラウンド GC の直前に走るエフェメラル GC は開始をバックグラウンド GC と共有し、終了だけ別に送られるためで、開いている GC が無いときの終了は捨てている。
このとき gen2 のゾーンはエフェメラル GC の終わりで閉じてしまい、バックグラウンド GC の本体は映らない。

GC が止めていた時間そのものは `EE suspend: GC` / `EE stopped` / `EE wait: GC` の方が正確で、
`GC` レーンは「いま何世代の GC が走っているか」を見るためのものと考える。
...
GC をまだスレッド上のゾーンにしていた頃、**バックグラウンド gen2 GC はサスペンドの内側で始まり、世界が再開した後も続く**ので、
`GC gen2` と `EE stopped` が同一スレッド上で交差した。
...
そこで、交差するゾーンを**いったん閉じて即座に開き直す**ようにした。
上に積まれていたゾーンは隣接する区間に分かれ、継続分には `continued` のテキストが付く。
時間の被覆は正しいまま、スタック規律も守られる。
GC をスレッドに出していた頃の 8 秒のキャプチャで、分割が起きたのは 10 回だった。

GC は今はレーンに出している（上記）ので、この交差そのものはもう起きない。
分割の仕組みは、同じスレッド上で順序が保証されないものが残ったときのために置いてある。
...
`SetEnterLeaveFunctionHooks3` が落ちるのはランタイムの不具合ではなく、こちらの作り方の問題である。
ELT3 のフックは高速経路で、JIT されたコードから間に何も挟まずに直接呼ばれる
（`EEToProfInterfaceImpl::DetermineAndSetEnterLeaveFunctionHooksForJit` がフックのアドレスをそのまま JIT に渡す）。
そのためフック自身が全レジスタを保存しなければならず、ドキュメントも naked 関数での実装を求めている。
普通の C++ 関数は呼び出し元の引数レジスタを壊すので、その先で何が起きてもおかしくない。
`SetEnterLeaveFunctionHooks2` の方はランタイム側のレジスタ保存スタブ（`ProfileEnterNaked`）と `ProfileEnter` を経由して普通に呼ばれるので、C++ 関数のままで動く。
...
enter フックはそれを引く。メタデータを読んでよい場所はここだと文書化されており、フックの中から読むのは論外。
表はロックフリーのオープンアドレス法（書き手は JIT だけ、読み手は全スレッドの全呼び出し）。
マッパーを使えばクライアント ID に call site ポインタを入れてフックの引きを無くせる（呼び出しあたり 1 回のハッシュ引きが消える）が、
...
除外されたメソッドは名前の表に番兵として入り、入ったときにゾーンを開かない（後述のフレームだけを積む）。
...
#### ゾーンを開くかは入ったときに決め、戻るときはそれに従う

「このフレームにゾーンがあるか」の答えは、enter と、それに対応する leave・tailcall・例外の巻き戻しで
完全に一致していなければならない。片方だけが push / pop すればそこで壊れる。

最初の実装は 4 経路がそれぞれ名前の表を引いて判定していたが、これでは一致しない。
名前が表に入るのは `JITCompilationFinished` だが、メソッドのコードはその通知より前に他のスレッドから呼ばれうる。
enter の時点では表に無く（何も積まない）、leave の時点では表にある（pop する）と、呼び出し元のゾーンを閉じてしまう。

今は、フックの付いた enter は**必ずフレームを 1 つ積む**。
名前がまだ無い、除外されている、深さの上限を超えた、のどれかならゾーンは開かず、FunctionID だけを持った印になる。
leave と tailcall は、一番内側のフレームがその FunctionID のものなら pop する。
表はもう引かないので enter と答えが食い違うことは無く、leave は以前よりむしろ安い（ハッシュ引きが無くなった）。
...
一番内側のフレームの FunctionID が一致するときだけ pop する。無条件に pop すると他人のゾーンを閉じてしまう。

名前の表は「このフレームを何と呼ぶか」にだけ答える。
名前が読めなかったメソッドは総称名（`managed`）で記録し、
`DynamicMethod` 由来（IL スタブ、ラムダ）はメタデータが無いので `DynamicMethodJITCompilationFinished` で総称名を入れている。
...
この表は GC をまだスレッド上のゾーンにしていた頃のもので、`GC gen*` の行は当てにならない
（バックグラウンド GC の開始と終了は別スレッドに来るので、ゾーンが正しい所で閉じていない）。
GC をレーンに移し、`EE wait` を足した後の計測はまだ取っていない。
...
- **GC レーンと `EE wait` を実際のキャプチャで確かめる**。ソースを読んで組んだだけで、まだ走らせていない。
  特に `DOTNET_gcServer=1` で `Instrumentation failure` が出ないこと、GC レーンが UI に出ること、
  ワーカーに `EE wait: GC` が並ぶことを見る
...
- GPU ゾーンはレンダラができてから（GPU コンテキスト ID 255 は GC レーンが使っている）
