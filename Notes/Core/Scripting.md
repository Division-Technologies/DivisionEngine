...
1. **Capture**: インスタンス化済みの全スコープを現在の状態でシリアライズしてメモリに退避し、参加者（`IReloadParticipant`。エンティティワールドなら `WorldReloadParticipant`）にも書き出させる。まだ何も壊していないので、ここで失敗してもリロードを諦めるだけで何も変わらない
2. **Release**: ライブオブジェクトグラフを破棄し（スコープ・ローダを全クリア）、参加者にもユーザー型への参照を手放させる（ワールドを空にし、コンポーネント型を登録解除）。これで旧 ALC への参照が消える
3. `ScriptHost.Swap` で ALC を差し替える。以後の型解決は新しい `TypeResolver` を経由させる。新しい DLL を読めなかった場合（アセンブリでない、モジュール初期化子が投げた）は、読みかけの分をアンロードして**直前のアセンブリを読み直す**
4. 退避したバイト列から `FileScopeLoader` を作ってスコープを再構築し、全オブジェクトを解決 → `DrainPending` でデシリアライズする。読めなくなったアセットはエラーを出して飛ばし、他のアセットは続ける
5. **Restore**: 参加者に状態を戻させる（アセットの解決に使ったリゾルバを渡す）。3 で差し替えに失敗していた場合は、直前のアセンブリに対して戻したあとで `ScriptReloadException` を投げる

`ScriptsDirty` は試す前に下ろす。失敗したリロードは直前のアセンブリへ戻っているので、同じ DLL で毎フレーム再試行しても同じ失敗を繰り返すだけになる。次のコンパイルがフラグを立て直す。
