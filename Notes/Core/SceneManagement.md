...
- 階層（Transform 親子）は特権構造にせず、コンポーネント（`Parent`/`Children` 相当）として表現する。Transform 伝播はシステムの 1 つ
...
- 階層整合性（親の破棄時の子の扱い）の仕様
- レンダリングミラーの更新方式（差分コマンド vs Extract コピー）
- ラウンドのラベル列を登録する API と、System / Behaviour がラベルを参照する構文
