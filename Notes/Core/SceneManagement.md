...
### 未解決: Transform 伝播の並列幅はルートのチャンク数で決まる

`TransformPropagationSystem` はルート（`LocalTransform` + `WorldTransform` かつ `Parent` なし）を
チャンク並列で走査し、各バッチが自分の部分木を降りる。分割の単位が**ルートのチャンク**なので、
並列幅はルートが何チャンクを占めるかで決まり、階層に何エンティティぶら下がっているかとは無関係になる。
...
**128 ルートを切ると work item が 1 個になり、伝播が丸ごと直列になる。**

ただしこれは「全ルートが同一アーキタイプ」という条件での話で、実シーンほど悲観的ではない。
ルートの持つコンポーネントが違えばアーキタイプが分かれ、チャンクも分かれるからである。
ストレージを持たないタグで分けるだけ（stride は不変）で、100 ルートでも並列性は戻る:
...
**しかし幅が戻っても粒度の問題は残る。** 16 アーキタイプに分けたうえで、1 ルートが全体の 80% を
抱える形（レベルルート + 小物、という普通のシーン）にすると 1.26x まで落ちる。
仕事の 80% が分割不能な 1 work item に入るので Amdahl の上限 1.25x に張り付く。

両方を貫く原因は 1 つで、**スケジューラは部分木を割れない**。アーキタイプの多様さは
「ルートの並び」を細かく割ってくれるが、1 本の部分木は何をしても 1 つの work item のまま。
結果として並列性がオーサリングの副作用になり、「全ルートに共通のコンポーネントを足す」ような
普通のリファクタが原因の見えないフレームレート低下を生む。

対処の候補は 2 つあり、どちらも部分木を割れるようにするもの:

- 部分木のワークスティーリング分割
- 深さ（`Depth`）を持たせた幅優先化

なお伝播コストの内訳は**走査が 83%、変換計算が 17%**（10 万エンティティ 1 スレッドで、
フラット 924 µs 対 階層 7,413 µs。[Profiling.md](./Profiling.md)）。
...
静的な Transform を安くするなら「計算を飛ばす」ではなく「訪問しない」必要がある
（静的部分木のリンク切り離し、`Without<Static>` でのクエリ除外）。
