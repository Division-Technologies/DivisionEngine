...
- クエリではなく**実行時に決まる個数の work item** に分けたい場合は `ScheduleBatches(name, itemCount, access, body)`。`itemCount` はチャンク集合と同じくノードが ready になった時点で読むので、**先行ジョブが作ったリストの長さを見られる**。分けたい仕事がクエリで表せないときの受け皿で、Transform 伝播が部分木を繰り延べるのに使っている（[SceneManagement.md](./SceneManagement.md)）。同じ資源を write する複数ノードは直列化されるため、「同じ仕事を N 個のジョブに分けて並列に」は `Schedule` を N 回呼んでも実現できない。並列になるのは 1 ノードの中の work item 同士だけ
...
- 例外はノードに記録してスケジューラに報告し、次の `Wait` / `WaitAll` で `JobFailedException` として再スローする。ワーカーは止まらない。`ScheduleBatches` の `itemCount` が投げた場合も同じ経路に乗り、ノードは失敗として完了して後続を解放する（以前は `WaitAll` が永久に待つか、ワーカーのスレッドが落ちた）
- `EndPhase` は失敗があってもフェーズを閉じきる。他のターンの書き込みのコミットと構造変更の適用を済ませてから、集めた失敗を再スローする。System のスケジュール中に例外が出て `ResumePhase` に届かなかった場合は、`BeginPhase` が取った取り込みと待機ターンを戻し、次のフェーズで再開する
- コマンドバッファの playback は、既に消えたエンティティへの `DestroyEntity` を no-op にする（2 つの Behavior が同じ敵を倒すのは通常のゲームプレイ）。それ以外に失敗したコマンドは飛ばして残りを適用し、バッファは必ずクリアしたうえで `EntityCommandBufferPlaybackException` にまとめて投げる。`EndPhase` はバッファごとに失敗を集め、後続のターンのバッファも適用する
...
- 外部 await と `RunBackground` の完了は取り込みキューに積まれ、フレーム冒頭の `AdmitExternal` で TurnId 順に取り込まれる
- **ターンの終わり方**: エンティティの破棄、Behavior コンポーネントの削除・別インスタンスへの差し替え、失敗（同期・非同期どちらの例外も）はいずれもターンをキャンセルする。コンポーネントの削除は、構造が変わったときだけディスパッチフェーズの `BeginPhase` で検査する（セグメントの中からは構造を読めないため）。失敗したターンはキャンセル扱いなので、コンポーネントが残っていれば `BehaviorStartSystem` が次のフレームで最初から始め直す
- **Behavior を再開しないフェーズへの `await ctx.Phase(...)` は、その場で `InvalidOperationException`**。`Engine` がループのフェーズを `JobGraph.DeclarePhase` で宣言しており、宣言にないフェーズやディスパッチしないフェーズ（Physics など）を待つと永久に戻らないため
...
- **取り込み（開始・繰り延べ・外部完了・completed の読み）も `ResumePhase` で発行する**。以前は `BeginPhase` で発行しており、System の発行より先に走り出した最初のセグメントが次に発行する read が、System の write より前に並ぶことがあった。0 ワーカーでは System の後の値、ワーカーありでは前の値が見え、`initial` の定義が崩れていた
- **`Start` はメインスレッド専用**。TurnId がコミット順を決めるので、採番がタイミングに依存してはならない。Behavior から別の Behavior を始めるときは、`Commands` でコンポーネントとして追加する
- **`Start` は常に次のディスパッチフェーズで TurnId 順に発行される**。以前の「フェーズ外なら即時」は、0 ワーカーでは次の `WaitAll` まで走らないため、Behavior が動き始めるフレームがワーカー数に依存していた。決定性ハーネスが最初に検出した非決定性がこれ
