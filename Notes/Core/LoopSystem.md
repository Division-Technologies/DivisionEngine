...
実装済みの範囲: 外部 await と `RunBackground` の完了は `JobGraph` の到着キューに溜まり、`Engine.RunFrame` の冒頭で `AdmitExternal` が (TurnId, ターン内連番) のキー順に取り込む。連番は await を開始した時点でセグメント内で採番するので、キーは実行タイミングに依存しない。`FrameLog` は各フレームの `Realtime` と取り込んだキー列を記録し、`Engine.Replay(log)` は記録どおりの時刻・キーで再生する（記録されたキーが未到着なら到着を待ち、記録外の到着は保留する）。時計は巻き戻さないので、リプレイは 1 フレームも回していない `Engine` でしか始められない。リプレイ中は時刻をログから取るため、`RunFrame(Realtime)` は使えない。OS 入力・ネットワークは未実装で、同じ枠組みに載せる予定。
