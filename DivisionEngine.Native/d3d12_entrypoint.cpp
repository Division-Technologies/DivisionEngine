...
DIVISION_EXPORT HRESULT InitD3D12(HWND hWnd, UINT window_width, UINT window_height)
{
	...
	result = _dxgiFactory->EnumAdapters(0, &_dxgiAdapter);
	if (result != S_OK)
		return result;

	result = _dxgiAdapter->EnumOutputs(0, &_dxgiOutput);
	if (result != S_OK)
		return result;
	...
	result = _dxgiFactory->CreateSwapChainForComposition(_cmdQueue, &swapchainDesc, nullptr, (IDXGISwapChain1**)&_swapchain);
